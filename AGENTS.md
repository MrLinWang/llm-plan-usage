...
- **LLM Gateway**: Live provider configured entirely in `config.toml` like every other platform — `base_url` (required) + `api_key` (single-key form). No side config files. Extracts today's USD cost from `usage.today.actual_cost` (fallback: `usage.daily_usage`/top-level `daily_usage` last item, then legacy `cost`). Two modes switched by `use_groups` (`true` = groups only, `false` = single key only, absent = groups win if configured; `false` ignores stored groups): single-key form uses the top-level `api_key`; multiple keys sharing one daily quota use `[[platforms.llm-gateway.groups]]` (`name`, `daily_limit`, `api_keys`/`keys`/`api_key_envs`) instead of the top-level `api_key`; **Web `/config` is group-only**: `CREDENTIAL_FIELDS["llm-gateway"]` is empty, the editor defaults to one group (组1) with one key input + one daily-limit input per group, 「添加共享 Key」 appends keys sharing that group's quota, 「添加组」 adds groups, and saving writes `groups` plus `use_groups = true` (overriding any legacy `use_groups=false`); the card also edits `base_url` (PUT `base_url` is gateway-only: non-empty string writes, empty = keep; view returns it). Legacy top-level single-key form (`api_key` + optional `daily_limit` + `use_groups`) stays supported by the provider for hand-edited `config.toml`; the web view synthesizes it as one group (组1) and the first save migrates it to `groups` (empty `groups` PUT → 400). `usage_path` etc. stay hand-edited in `config.toml`. each group fans out one request per key (`ThreadPoolExecutor`, capped at `MAX_KEY_WORKERS=8`) and sums the successful keys' `actual_cost` into one `UsageEntry` per group. Duplicate keys within a group are stably de-duped; a key assigned to more than one group is a config error. Per-key failures become `PlatformResult.warning` (never raised) as long as at least one key in a group succeeds; a group with zero successes contributes no entry; all groups failing (or the single key failing) sets `PlatformResult.error`. A non-`None` `warning` means the aggregate is partial, so `store.save_snapshot()` skips it — partial data is never persisted as a full historical snapshot.
...
| `src/llm_usage/static/config.html` | Admin provider config — one card per platform (`enabled` checkbox + credential-slot editor: each slot = one billing plan with 套餐名 input + credential inputs, 「添加凭证」/「添加 AK/SK」/删除槽, `env:`/masked/unset placeholders, empty = keep), PUT per card; LLM Gateway card has a `base_url` input (empty = keep) + pure group editor (default 组1, one key input + daily limit per group, 「添加共享 Key」/「添加组」/删除分组, masked keep-markers) |
...
| `src/llm_usage/providers/llm_gateway.py` | LLM Gateway live provider — pure `config.toml` config (`base_url` + `api_key`), calls `/v1/usage`, displays today's `actual_cost` in USD; supports `groups` for API keys sharing one daily quota |
